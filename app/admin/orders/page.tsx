    ...
    <div>
      ...
      <div className="mb-6">
        <select
          ...
          className="px-4 py-2 bg-white/10 border border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500"
        >
        ...
        <div className="space-y-4">
          {filteredOrders.map(order => {
            ...
            return (
              <div 
                key={order.id} 
                className="bg-white/5 backdrop-blur-sm rounded-xl border border-purple-500/20 overflow-hidden hover:border-purple-500/40 transition-all cursor-pointer"
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-3">
                        <h3 className="text-xl font-bold">Order #{order.orderId || order.id.slice(0, 8)}</h3>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                        ...
                        <div>
                          ...
                          <p className="font-medium">{order.eventName || order.event?.name || 'N/A'}</p>
                        </div>
                        
                        <div>
                          <p className="text-gray-400 mb-1">Tickets</p>
                          <p className="font-medium">{order.tickets?.length || order.quantity || 0}</p>
                        </div>
      ...
      {selectedOrder && (() => {
        ...
        return (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-gradient-to-br from-purple-900 via-black to-black border border-purple-500/30 rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
              <div className="sticky top-0 bg-gradient-to-r from-purple-900 to-black border-b border-purple-500/30 p-6 flex justify-between items-center">
                <div>
                  <h2 className="text-2xl font-bold">Order Details</h2>
                  <p className="text-gray-400">#{selectedOrder.orderId || selectedOrder.id.slice(0, 8)}</p>
                </div>
                <button
              ...
              <div className="p-6">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  ...
                  <div className="space-y-6">
                    {/* Customer Info */}
                    <div>
                      <h3 className="text-sm font-semibold text-purple-400 mb-3">CUSTOMER DETAILS</h3>
                      <div className="bg-white/5 p-4 rounded-lg space-y-2">
                        <p><span className="text-gray-400">Name:</span> <span className="font-medium">{customerInfo.name}</span></p>
                        <p><span className="text-gray-400">Email:</span> <span className="font-medium">{customerInfo.email}</span></p>
                        <p><span className="text-gray-400">Phone:</span> <span className="font-medium">{customerInfo.phone}</span></p>
                      </div>
                    </div>
                    ...
                    {selectedOrder.event && (
                      <div>
                        <h3 className="text-sm font-semibold text-purple-400 mb-3">EVENT DETAILS</h3>
                        <div className="bg-white/5 p-4 rounded-lg space-y-2">
                          <p><span className="text-gray-400">Event:</span> <span className="font-medium">{selectedOrder.event.name}</span></p>
                          <p><span className="text-gray-400">Venue:</span> <span className="font-medium">{selectedOrder.event.venueName || 'N/A'}</span></p>
                          {selectedOrder.event.schedule?.performances?.[0]?.date && (
                            <p>
                              <span className="text-gray-400">Date:</span>{' '}
                              <span className="font-medium">{new Date(selectedOrder.event.schedule.performances[0].date).toLocaleDateString()}</span>
                            </p>
                          )}
                        </div>
                      </div>
                    ...
                    {/* Payment Info */}
                    <div>
                      <h3 className="text-sm font-semibold text-purple-400 mb-3">PAYMENT DETAILS</h3>
                      <div className="bg-white/5 p-4 rounded-lg space-y-2">
                        <div className="flex justify-between">
                          <span className="text-gray-400">Subtotal:</span>
                          <span>${(selectedOrder.pricing?.subtotal || selectedOrder.total || 0).toFixed(2)}</span>
                        </div>
                        {selectedOrder.pricing?.fees?.service > 0 && (
                          <div className="flex justify-between">
                            <span className="text-gray-400">Service Fee:</span>
                            <span>${selectedOrder.pricing.fees.service.toFixed(2)}</span>
                          </div>
                        )}
                  ...
                  <div>
                    ...
                    <div className="space-y-4">
                      {selectedOrder.tickets?.map((ticket: any, index: number) => (
                        <div key={ticket.id || index} className="bg-white/5 p-4 rounded-lg border border-purple-500/20">
                          <div className="flex justify-between items-start mb-3">
                            <div>
                              <p className="font-semibold">{ticket.tierName || 'General Admission'}</p>
