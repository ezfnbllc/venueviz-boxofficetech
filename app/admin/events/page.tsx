...
export default function EventsPage() {
  ...
  const handleCloseWizard = () => {
    setShowWizard(false)
    setEditingEventId(undefined)
    loadEvents() // Reload events after closing wizard
  }
      ...
      <div className="p-8 border-b border-white/10">
        ...
        <div className="flex gap-4">
          <input
            ...
            className="px-4 py-2 bg-white/10 rounded-lg flex-1 max-w-md"
          />
          <select
            value={filter}
          ...
          {filteredEvents.map(event => (
            <div key={event.id} className="bg-gray-900 rounded-xl overflow-hidden hover:shadow-xl transition-shadow">
              {event.images?.cover ? (
                ...
                <div className="w-full h-48 bg-gradient-to-br from-purple-600 to-pink-600"/>
              )}
