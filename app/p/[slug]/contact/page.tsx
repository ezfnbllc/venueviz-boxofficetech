...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const cmsPage = await getCMSPage(promoter.id, 'contact')

  return {
    title: cmsPage?.seo?.title || `Contact | ${promoter.name}`,
    description: cmsPage?.seo?.description || `Get in touch with ${promoter.name}`,
