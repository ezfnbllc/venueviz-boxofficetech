...
interface Order {
  ...
  seats: Array<{section: string; row: number; seat: number; price: number}>
  total: number
  qrCode: string
}
      ...
      <div className="max-w-2xl mx-auto">
        <div className="bg-black/40 backdrop-blur-xl rounded-xl border border-white/10 p-8 text-center">
          ...
          <div className="bg-white p-6 rounded-lg mb-8">
            <div className="text-black">QR Code: {order.qrCode}</div>
          </div>
          ...
          <button onClick={() => router.push('/')} className="w-full py-3 bg-purple-600 rounded-lg">
            Book More Events
          </button>
        </div>
      </div>
